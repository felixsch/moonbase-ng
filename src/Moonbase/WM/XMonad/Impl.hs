...
{- This module is orginally written by Spencer Jansson 2007
 - I just modified some code to make it work with fork (aka remove resume/recompile/replace code)
 -
 - The orginal source is contributed under the BSD3-style licence. Checkout LICENCE-BSD for more infomation
 -
 - Adapted from here: https://hackage.haskell.org/package/xmonad-0.11/docs/src/XMonad-Main.html#xmonad
 -}

module Moonbase.WM.XMonad.Impl
  ( moonbaseXMonad 
  ) where
...
--
moonbaseXMonad :: (LayoutClass l Window, Read (l Window)) => XConfig l -> IO ()
moonbaseXMonad initxmc = do
    -- setup locale information from environment
    withCString "" $ c_setlocale (6)
{-# LINE 62 "XMonad/Main.hsc" #-}
    ...
    let layout = layoutHook xmc
        ...
        st = XState
            ...
            , dragging        = Nothing
            , extensibleState = M.empty -- extState
            }
    allocaXEvent $ \e ->
        runX cf st $ do
            ...
            -- restarting.
            windows . const . foldr W.delete initialWinset $ W.allWindows initialWinset \\ ws

            -- manage the as-yet-unmanaged windows
            mapM_ manage (ws \\ W.allWindows initialWinset) -- winset
